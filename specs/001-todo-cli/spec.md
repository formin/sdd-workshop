...
**Created**: 2026-05-02  
**Status**: Draft  
**Input**: User description: "CLI 기반의 ToDo 관리 앱을 만들고 싶어요. 사용자: 터미널을 사용하는 개인 개발자. 주요 기능: 1) ToDo 항목 추가: 제목(필수), 마감일(선택), 우선순위(선택) 2) 전체 목록 조회: 완료/미완료/우선순위로 필터링 가능 3) 항목 완료 처리: 항목 ID로 완료 표시 4) 항목 삭제: 항목 ID로 삭제. 기술스택은 아직 미정"
...
- 의존성: 외부 패키지 도입은 최소화; 필요한 경우 `requirements.txt`에 이유를 기록한다.
